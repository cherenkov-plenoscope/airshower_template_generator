...
if os.path.exists(path):
    ...
    results = np.zeros(shape=shape, dtype=np.float32)
        ...
        for azi in azi_range:
            for rad in rad_range:
                for alt in alt_range:
                    ...
                    moyal_fit = MoyalFit(
                        density_c_parallel=density_c_parallel[
                            ene, azi, rad, alt
                        ],
                        binning=binning,
                    )
                    ...
                    results[
                        ene, azi, rad, alt, IDX_MOYAL_PEAK_DEG
                    ] = mm.values["moyal_peak_deg"]
                    results[ene, azi, rad, alt, IDX_WIDTH_DEG] = mm.values[
                        "width_deg"
                    ]
                    results[
                        ene, azi, rad, alt, IDX_TAIL_DIRECTION
                    ] = mm.values["tail_direction"]
